...
K_PATCHES= 2.0 # Strength of patch attraction in kcal/mol/A^2
...
for i in range(N_GRANULES):
    granule= gf.create_granule_with_interaction_patches("Granule_{}".format(i),
                                                        N_GRANULE_PATCHES)
...
evr = IMP.core.ExcludedVolumeRestraint(IMP.atom.get_leaves(h_root),
                                      K_EXCLUDED,
                                      10, # slack affects speed only
                                          # (slack of close pairs finder)
                                      "EV")
rs.append(evr)
# Add interaction among all patch sites
...
print("Score after: {:f}".format(bd.get_last_score()))
